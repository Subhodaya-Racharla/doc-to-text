...
# Lines that are pure OCR / scanner noise
_NOISE_PATTERNS = [
    ...
    re.compile(r"^\s*[-_=*]{4,}\s*$"),       # separator lines like ---- or ====
    re.compile(r"^\s*\d+\s*$"),               # lone page numbers
    re.compile(r"^\s*.{0,2}\s*$"),            # lines with 0-2 chars (garbage)
]
...
def clean_ocr_text(text: str) -> str:
    ...
    for line in lines:
        ...
        if any(p.match(line) for p in _NOISE_PATTERNS):
            continue
        # Collapse multiple spaces within a line but preserve the line itself
        line = re.sub(r"[ \t]+", " ", line).rstrip()
        cleaned.append(line)

    # Merge broken lines: if a line ends without punctuation and the next line
    # starts with a lowercase letter, join them (common OCR artifact)
    merged = []
    for i, line in enumerate(cleaned):
        ...
        if not stripped:
            merged.append("")
            continue
        # Check if previous non-empty line should be joined with this one
...
# Comprehensive alias map — covers IDs, medical, insurance, invoices, forms
...
def _field_variants(field: str) -> list[str]:
    ...
    # Add known aliases (bidirectional lookup)
    ...
    # Also try partial matching: if user typed "claim" and we have "claim number"
    for key, aliases in _ALIASES.items():
        nk = _normalize(key)
        all_forms = [nk] + [_normalize(a) for a in aliases]
        for form in all_forms:
            # If user field is a substring of an alias or vice versa
            if len(norm) >= 3 and (norm in form or form in norm):
                variants.update(all_forms)
                break

    # Build regex patterns from each variant
    result = []
    for v in sorted(variants, key=len, reverse=True):  # longer patterns first
        words = v.split()
        parts = []
        for w in words:
            # Allow OCR misspellings: c↔e, i↔l, s↔5, o↔0 via char substitution
...
def _find_field_position(text: str, field: str) -> re.Match | None:
    ...
    # Strategy 1: field label followed by an explicit separator (: = - tab)
    for vp in variants:
    ...
    # Strategy 2: field label at start of line followed by multiple spaces (tabular)
    for vp in variants:
    ...
    # Strategy 3: field label at end of line (value on next line)
    for vp in variants:
        pattern = rf"(?im)({vp})\s*$"
        m = re.search(pattern, text)
        if m:
    ...
    # Strategy 4: field label anywhere in a line (less strict, for noisy OCR)
    for vp in variants:
        pattern = rf"(?i)({vp})"
        m = re.search(pattern, text)
        if m:
            # Only accept if followed by some separator-like pattern within a few chars
...
# ═══════════════════════════════════════════════════════════════════════════════
# MODULE 4: Multi-line Value Extraction
# ═══════════════════════════════════════════════════════════════════════════════
...
def _is_field_label(line: str, all_field_patterns: list[str] | None = None) -> bool:
    ...
    # Mixed-case or uppercase label followed by separator with a value
    if re.match(r"^[A-Za-z][A-Za-z\s./#]{1,50}\s*[:=]\s*\S", stripped):
        return True
    # ALL-CAPS label followed by separator (with or without immediate value)
    if re.match(r"^[A-Z][A-Z\s./#]{1,50}\s*[:=\-]\s*", stripped):
        return True
    # Table-like: pipe-separated columns
    if stripped.count("|") >= 2:
        return True
    # If we have field patterns from user, check if this line matches any
...
def _extract_multiline_value(text: str, start_pos: int,
    ...
    """Extract value starting at start_pos, spanning multiple lines if needed."""
    ...
    while start_idx < len(lines) and not lines[start_idx].strip():
        start_idx += 1
        if start_idx > 2:  # don't skip more than 2 blank lines
            return ""
    ...
    for i in range(start_idx, min(len(lines), start_idx + max_lines)):
        ...
        # Stop conditions (only after collecting at least one line)
        if value_parts:
            # Empty line = end of value block
            if not stripped:
                break
            # Next field label detected
            if _is_field_label(stripped, all_field_patterns):
                break

        if stripped:
            value_parts.append(stripped)

        # For inline values (value on same line as label), check if next line continues
        if i == start_idx and value_parts:
            if i + 1 < len(lines):
                next_s = lines[i + 1].strip()
                if not next_s:
                    break  # blank line = single-line value
                if _is_field_label(next_s, all_field_patterns):
                    break  # next line is a new field
                # Otherwise continue to next line (multi-line value)
            else:
                break
...
# ═══════════════════════════════════════════════════════════════════════════════
# MODULE 5: Table Detection & Extraction
# ═══════════════════════════════════════════════════════════════════════════════
...
def _extract_table_rows(text: str) -> list[dict[str, str]]:
    ...
    if len(pipe_lines) >= 2:
        ...
        for j in range(1, len(pipe_lines)):
            if pipe_lines[j][0] - pipe_lines[j - 1][0] <= 2:  # allow 1 gap line
...
# ═══════════════════════════════════════════════════════════════════════════════
# MODULE 6: Main Field Extraction (combines all modules)
# ═══════════════════════════════════════════════════════════════════════════════

def extract_field_value(text: str, field: str,
                        all_field_patterns: list[str] | None = None) -> str:
    """Extract a field value from text using the full extraction pipeline."""
    # First try key-value extraction from cleaned text
...
async def _extract_rows(files: List[UploadFile], fields: str) -> tuple[list[str], list[dict]]:
    ...
    for file in files:
        ...
        # Pre-compute all field patterns so multi-line extractor knows where fields start
        all_patterns = []
        for f in field_list:
        ...
        for field in field_list:
            row[field] = extract_field_value(text, field, all_patterns)
        rows.append(row)
